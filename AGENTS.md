...
4. **Apifox 契约优先：** 接口变更应先更新 Apifox 文档，再同步修改前后端代码（契约权威见 3.2 红线 7）
...
Agent 创建新分支时，只允许以下五类前缀，格式 `{前缀}/{模块名}-{功能简述}`（小写英文、横线分隔）；一个分支只表达一种变更类型，不得直接在 `master` 或 `develop` 上 commit。完整示例与操作步骤见 [README.md 分支命名规范](./README.md#分支命名规范)。
...
- 修改 `docs/` 下任何公共文档（project / development / operations / deliverables）
...
`backend/`、`frontend/` 是全体成员的开发位置，**允许直接修改**，但修改者必须承担前后端一致性责任（接口联动与联调验证规则见 2.2 前后端一致性规则）：

1. **更新规范：** 若修改涉及开发流程、目录结构或启动方式的变化，同步更新 `docs/development/` 中对应的规范文档
2. **分离关注点：** 结构性改造（如新增中间件、调整分层架构）放在独立 PR 中，不与业务功能混合

SET DEFINE OFF;
SET AUTOCOMMIT ON;
-- 04 安全社区：门禁/晚归/访客授权/投票+响应/违规
-- 依赖：01/03 已执行。
-- 链路：C8 安全社区（访客二维码/投票）、ACCESS-01/02 门禁与楼内密度。
-- 041 起：D_Visitor_Log / D_Parcel_Record / D_Water_Order 已删除，
-- 原第 4/5/8 节样本随之下线（快递 C2-004/C8-003 校验样本口径由 PM 同步）。
...
-- ===== 4. 房间投票 D_Room_Vote + D_Room_Vote_Response（一人一票复合主键） =====
...
-- ===== 5. 违规 D_Violation_Record（联动信用分扣分，见 05 信用分） =====

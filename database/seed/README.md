...
前提：`foundation/001_create_tables.sql` 与 `extensions/010~030*.sql` 已建库。
...
1. **辅导员角色**：`IT_COUN_001` 按代码口径写 `Role_Level='辅导员'`。若目标库已启用 `CK_D_ADMIN_ROLE` 三值约束（楼长/维修员/超级管理员），会 ORA-02290 —— 需登记矛盾清单后处理（风险 R1）。

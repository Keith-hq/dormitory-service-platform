...
11. `ddl/extensions/019_vote_visitor_parcel_sequences.sql`
12. `verify/foundation_schema_checks.sql`
13. `verify/extension_schema_checks.sql`

`010_extension_tables.sql` 是一次性建表脚本。若表已存在，请使用全新的 schema 或容器进行复现，不要通过删表来绕过依赖问题。
`011` 至 `019` 是按编号顺序执行的增量迁移；已有环境只执行尚未应用的迁移，

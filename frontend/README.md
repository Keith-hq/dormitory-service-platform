...
高校宿舍后勤与共享生活服务平台的 Vue 3 前端项目。所有业务页面在此工程上开发，统一复用路由、状态管理、请求层和通用组件。
...
生产包统一通过 `npm ci && npm run check` 生成，构建产物为 `dist/`，不得提交到 Git。环境变量、产物验收、子路径部署和回滚要求见[《前端构建与发布规范》](../docs/development/05-前端构建与发布规范.md)。

...
> **契约权威来源：** Apifox 中已锁定的接口定义（最新导出的 OpenAPI 文件）为唯一契约权威；后端 Controller 路由、前端 `src/api/` 与本文档示例均以契约对齐，冲突时以 Apifox 为准。本文档只说明联调流程与示例，不替代契约本身。
...
编辑字段均可选；字段一旦传入，楼栋名称和类型不得为空白，楼层数必须在 1~50 之间。
